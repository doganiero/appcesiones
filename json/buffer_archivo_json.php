...
if (strpos($canal_selnom, "XCOM") !== false)
    $strcanal = " v_cargas_archivo.CARGA_DESCANAL LIKE '%XCOM%' ";
else if (strpos($canal_selnom, "FTP") !== false) {
    $strcanal = "  v_cargas_archivo.CARGA_DESCANAL LIKE '%FTP%' ";
} else
    $strcanal = " v_cargas_archivo.CARGA_CODCANAL='$codcanal' ";
...
$campos = array(
    0 => 'V_BUFFER_ARCHIVO.CODCARGA',
    1 => 'V_BUFFER_ARCHIVO.BUFFER_DESCANAL',
    2 => 'CODBUFFER',
    3 => 'V_BUFFER_ARCHIVO.CODENVIO',
    4 => 'CODFICHERO',
...
if ($mascara) {
    ...
    if (!$ipdest)
        $ipdest = 0;
    if ($codcarga)
        $result = mysql_query("SELECT BUFFER_FICHERO_ORIGEN AS count FROM $tabla, v_cargas_archivo WHERE BUFFER_FICHERO_ORIGEN LIKE '$mascara' AND v_cargas_archivo.CODCARGA=V_BUFFER_ARCHIVO.CODCARGA AND $strcanal AND v_cargas_archivo.CODCARGA='$codcarga' 
                                AND (V_BUFFER_ARCHIVO.DESEMPRESA_DESTINO IN (SELECT DESEMPRESA FROM v_envios WHERE (CODEMPRESAPADRE='$codempresa' OR CODEMPRESA='$codempresa' )) OR V_BUFFER_ARCHIVO.BUFFER_IP_DESTINO='$ipdest' OR V_BUFFER_ARCHIVO.BUFFER_NOMBREMAQUINA_DESTINO='$ipdest') ");
    else
        $result = mysql_query("SELECT BUFFER_FICHERO_ORIGEN AS count FROM $tabla, v_cargas_archivo WHERE BUFFER_FICHERO_ORIGEN LIKE '$mascara' AND v_cargas_archivo.CODCARGA=V_BUFFER_ARCHIVO.CODCARGA AND $strcanal  
                                AND (V_BUFFER_ARCHIVO.DESEMPRESA_DESTINO IN (SELECT DESEMPRESA FROM v_envios WHERE (CODEMPRESAPADRE='$codempresa' OR CODEMPRESA='$codempresa' )) OR V_BUFFER_ARCHIVO.BUFFER_IP_DESTINO='$ipdest' OR V_BUFFER_ARCHIVO.BUFFER_NOMBREMAQUINA_DESTINO='$ipdest') ");
}
...
if ($codbuffermask) {
    $sql_pos = "SELECT * FROM (SELECT @rownum:=@rownum + 1 as row_number, 
       t.*
FROM ( ";
    $sql_pos.="select CODBUFFER from v_buffer_archivo WHERE CODCARGA='$codcarga' GROUP BY BUFFER_DESCANAL, BUFFER_FICHERO_ORIGEN, DESEMPRESA_DESTINO,BUFFER_NOMBREMAQUINA_DESTINO, BUFFER_IP_DESTINO  ORDER BY $sidx $sord ";
...
if ($mascara) {
    ...
    foreach ($json as $rowm) {
        ...
        //------------------------------  SELECT V_BUFFER_ARCHIVO.CODCARGA, V_BUFFER_ARCHIVO.BUFFER_DESCANAL, CODBUFFER, V_BUFFER_ARCHIVO.CODENVIO, CODFICHERO, BUFFER_FICHERO_ORIGEN, CHKTRANSMISION_INTERNA, TRANSMISION_INTERNA, CODTIPOTRANSMISION, DESTIPOTRANSMISION, CHKCONTROL, DESEMPRESA_DESTINO, DESESTADOCONTROL, BUFFER_CHKCIFRADO, BUFFER_CIFRADO, CODTIPODESTINO, DESTIPODESTINO, DESEMPRESA_ORIGEN, BUFFER_NOMBREMAQUINA_ORIGEN, BUFFER_IP_ORIGEN, BUFFER_NOMBREMAQUINA_DESTINO, BUFFER_IP_DESTINO, MOTIVOLOG, BUFFER_FICHERO_DESTINO, BUFFER_FECHA_ALTA, BUFFER_FECHA_MODIFICACION, BUFFER_FECHA_BAJA, BUFFER_CODUSUARIO_ALTA, BUFFER_DESUSUARIO_ALTA, BUFFER_CODUSUARIO_MODIFICACION, BUFFER_DESUSUARIO_MODIFICACION, BUFFER_CODUSUARIO_BAJA, BUFFER_DESUSUARIO_BAJA, BUFFER_ESTADO, V_BUFFER_ARCHIVO.CODCARGA, CODBUFFER, BUFFER_FICHERO_ORIGEN FROM V_BUFFER_ARCHIVO , v_cargas_archivo WHERE BUFFER_FICHERO_ORIGEN LIKE 'A%' AND v_cargas_archivo.CODCARGA=V_BUFFER_ARCHIVO.CODCARGA AND (  BUFFER_DESCANAL LIKE '%XCOM%'   OR  BUFFER_DESCANAL LIKE '%XCOM%'  ) AND v_cargas_archivo.CODCARGA='000254'  
              //   AND( (V_BUFFER_ARCHIVO.DESEMPRESA_DESTINO IN (SELECT DESEMPRESA FROM tempresas WHERE CODEMPRESA='000000' OR CODEMPRESAPADRE='000000') AND (V_BUFFER_ARCHIVO.BUFFER_IP_DESTINO='' OR V_BUFFER_ARCHIVO.BUFFER_NOMBREMAQUINA_DESTINO='') )
                 
                // 		OR (V_BUFFER_ARCHIVO.DESEMPRESA_DESTINO IN (SELECT DESEMPRESA FROM tempresas WHERE CODEMPRESA='000000' OR CODEMPRESAPADRE='000000') AND (V_BUFFER_ARCHIVO.BUFFER_IP_DESTINO='' OR V_BUFFER_ARCHIVO.BUFFER_NOMBREMAQUINA_DESTINO='') )
                 //)
        ...
        if ($codcarga) {
            ...
            $codcanalaux2 = mysql_result(mysql_query("SELECT CARGA_CODCANAL FROM v_cargas_archivo WHERE CODCARGA='" . $codcarga . "' "), 0, 'CARGA_CODCANAL');
        ...
        if (!$primerop1) {
            ...
            if($codempresa!=="00000" && $codempresa!=0)$sqlsiemp="V_BUFFER_ARCHIVO.DESEMPRESA_DESTINO IN (SELECT DESEMPRESA FROM tempresas WHERE CODEMPRESA='$codempresa' OR CODEMPRESAPADRE='$codempresa') AND ";
            if (!$primerop2) {
                if($ipdest!==0) $parte2.="($sqlsiemp(V_BUFFER_ARCHIVO.BUFFER_IP_DESTINO='$ipdest' OR V_BUFFER_ARCHIVO.BUFFER_NOMBREMAQUINA_DESTINO='$ipdest') ) ";
                else $parte2.="($sqlsiemp ((V_BUFFER_ARCHIVO.BUFFER_IP_DESTINO IS NULL OR V_BUFFER_ARCHIVO.BUFFER_IP_DESTINO='') AND  (V_BUFFER_ARCHIVO.BUFFER_NOMBREMAQUINA_DESTINO IS NULL OR V_BUFFER_ARCHIVO.BUFFER_NOMBREMAQUINA_DESTINO='')) ) ";
                $primerop2++;
            } else
                if($ipdest!==0) $parte2.=" OR ($sqlsiemp (V_BUFFER_ARCHIVO.BUFFER_IP_DESTINO='$ipdest' OR V_BUFFER_ARCHIVO.BUFFER_NOMBREMAQUINA_DESTINO='$ipdest') )  ";
                else $parte2.=" OR ($sqlsiemp ((V_BUFFER_ARCHIVO.BUFFER_IP_DESTINO IS NULL OR V_BUFFER_ARCHIVO.BUFFER_IP_DESTINO='') AND  (V_BUFFER_ARCHIVO.BUFFER_NOMBREMAQUINA_DESTINO IS NULL OR V_BUFFER_ARCHIVO.BUFFER_NOMBREMAQUINA_DESTINO='')) ) ";
           $sqlsiemp="";     
            
            $SQL = "SELECT $stringCampos FROM $tabla, v_cargas_archivo WHERE BUFFER_FICHERO_ORIGEN LIKE '$mascara' AND v_cargas_archivo.CODCARGA=V_BUFFER_ARCHIVO.CODCARGA AND ($parte1) AND v_cargas_archivo.CODCARGA='$codcarga'  
                 AND ($parte2) $agrupa ORDER BY $sidx $sord  LIMIT $start , $limit";
            $SQL_nolim = "SELECT $stringCampos FROM $tabla, v_cargas_archivo WHERE BUFFER_FICHERO_ORIGEN LIKE '$mascara' AND v_cargas_archivo.CODCARGA=V_BUFFER_ARCHIVO.CODCARGA AND ($parte1) AND v_cargas_archivo.CODCARGA='$codcarga'  
                            AND ($parte2) $agrupa ";
            $order = " ORDER BY $sidx $sord ";
        } else {
           ...
           $sqlsiemp=""; 
            ...
            $SQL = "SELECT $stringCampos FROM $tabla, v_cargas_archivo WHERE BUFFER_FICHERO_ORIGEN LIKE '$mascara' AND v_cargas_archivo.CODCARGA=V_BUFFER_ARCHIVO.CODCARGA  
                AND ($parte2) $agrupa ORDER BY $sidx $sord  LIMIT $start , $limit";
            $SQL_nolim = "SELECT $stringCampos FROM $tabla, v_cargas_archivo WHERE BUFFER_FICHERO_ORIGEN LIKE '$mascara' AND v_cargas_archivo.CODCARGA=V_BUFFER_ARCHIVO.CODCARGA   
                       AND ($parte2) $agrupa ";
            $order = " ORDER BY $sidx $sord ";
        }
        ...
        if ($_GET['_search'] == 'true' && isset($searchField)) {
            if ($codcarga) {
                 ...
                 $codcanalaux2 = mysql_result(mysql_query("SELECT CARGA_CODCANAL FROM v_cargas_archivo WHERE CODCARGA='" . $carga . "' "), 0, 'CARGA_CODCANAL');
        ...
        if (!$primerop1) {
          ...
          if($codempresa!=="00000" && $codempresa!=0 )$sqlsiemp="V_BUFFER_ARCHIVO.DESEMPRESA_DESTINO IN (SELECT DESEMPRESA FROM tempresas WHERE CODEMPRESA='$codempresa' OR CODEMPRESAPADRE='$codempresa') AND ";
           ...
           $sqlsiemp=""; 
                ...
                $SQL = "SELECT $stringCampos FROM $tabla, v_cargas_archivo WHERE $condicionBusq $andCondicion and BUFFER_FICHERO_ORIGEN LIKE '$mascara' AND v_cargas_archivo.CODCARGA=V_BUFFER_ARCHIVO.CODCARGA AND ($parte1) AND v_cargas_archivo.CODCARGA='$codcarga'  
                 AND ($parte2)   
                    $agrupa ORDER BY $sidx $sord  LIMIT $start , $limit";
                $SQL_nolim = "SELECT $stringCampos FROM $tabla, v_cargas_archivo WHERE $condicionBusq $andCondicion and BUFFER_FICHERO_ORIGEN LIKE '$mascara' AND v_cargas_archivo.CODCARGA=V_BUFFER_ARCHIVO.CODCARGA AND ($parte1) AND v_cargas_archivo.CODCARGA='$codcarga'  
                            AND ($parte2) $agrupa ";
                $order = " ORDER BY $sidx $sord ";
            } else {
           ...
           $sqlsiemp=""; 
                ...
                $SQL = "SELECT $stringCampos FROM $tabla, v_cargas_archivo WHERE $condicionBusq $andCondicion AND BUFFER_FICHERO_ORIGEN LIKE '$mascara' AND v_cargas_archivo.CODCARGA=V_BUFFER_ARCHIVO.CODCARGA   
                AND ($parte2) $agrupa ORDER BY $sidx $sord  LIMIT $start , $limit";
                $SQL_nolim = "SELECT $stringCampos FROM $tabla, v_cargas_archivo WHERE $condicionBusq $andCondicion AND BUFFER_FICHERO_ORIGEN LIKE '$mascara' AND v_cargas_archivo.CODCARGA=V_BUFFER_ARCHIVO.CODCARGA   
                       AND ($parte2) $agrupa ";
                $order = " ORDER BY $sidx $sord ";
            }
        }
