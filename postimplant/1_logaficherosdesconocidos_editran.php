...
$sql_ficheros = "SELECT 
                ...
                WHERE E.CODENVIO=F.CODENVIO AND
                      F.CODCANAL='00001'AND   
                       E.CODEMPRESA='00000' AND
                       B.BUFFER_FICHERO_ORIGEN LIKE CONCAT(F.FICHERO_ORIGEN,'%' )  AND
                       F.CODFICHEROPADRE IS NULL AND
                       F.CODUSUARIO_BAJA IS NULL
                       GROUP BY F.CODENVIO, F.FICHERO_ORIGEN, B.BUFFER_FICHERO_ORIGEN, B.DESEMPRESA_DESTINO ORDER BY F.CODENVIO ";
...
while ($fila = mysql_fetch_object($buffresult)) {
    ...
    if ($codempresa) {
        if ($aux_codenvio != $fila->CODENV) {
            $sql_update = "UPDATE TENVIOS SET CODEMPRESA='$codempresa' WHERE CODENVIO='" . $fila->CODENV . "' ";
            ...
            $repetido = mysql_result(mysql_query("SELECT COUNT(*) as cuenta FROM tficheros f,tenvios e where e.codenvio=f.codenvio AND trim(f.FICHERO_ORIGEN)='" . trim($forigen) . "' AND e.CODEMPRESA='$codempresa'  AND f.CODCANAL='$codcanal' "), 0, 'cuenta');
            if ($repetido == 0) {
...
echo "ENVÍOS CON EMPRESA DESCONOCIDA MODIFICADOS: $contemp<br>
      NUEVOS ENVÍOS REPLICADOS: $contenvio<br> 
      NUEVAS EMPRESAS AGREGADAS DESDE EL LOG: $contnewemp<br>    
      ENVÍOS CON EMPRESA DESCONOCIDA RESTANTES: $contnotemp
