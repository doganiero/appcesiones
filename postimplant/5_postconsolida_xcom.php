...
$sql_buff2="SELECT  CODCARGA, CODBUFFER, FICHERO_ORIGEN, DESEMPRESA_DESTINO FROM TBUFFER WHERE CHKCONTROL='N' AND CODCARGA='000001' "; //<---- falta group by fichero origen y desempresa
$buffresult2=mysql_query($sql_buff2);

while($fila=  mysql_fetch_object($buffresult2)){
...
if ($CHKCONTROL == 'S') {
    if (!(mysql_num_rows($result_fich))) { 
        ...
        if (!$ip_destino)
            $ip_destino = $nombre_maquinad;
        if ($nombre_maquinad) $ip_destino=$nombre_maquinad;
